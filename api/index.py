from http.server import BaseHTTPRequestHandler
import json
import firebase_admin
from firebase_admin import credentials, firestore
import urllib.request
import urllib.parse

# Firebase Credentials JSON configuration
firebase_config = {
  "type": "service_account",
  "project_id": "active-winoffice-diefcosystem",
  "private_key_id": "21051d5be75ea0bca1bec554841fea50a1dcfcba",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  "client_email": "firebase-adminsdk-fbsvc@active-winoffice-diefcosystem.iam.gserviceaccount.com",
  "client_id": "117763845775781682477",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-fbsvc%40active-winoffice-diefcosystem.iam.gserviceaccount.com",
  "universe_domain": "googleapis.com"
}

if not firebase_admin._apps:
    cred = credentials.Certificate(firebase_config)
    firebase_admin.initialize_app(cred)

db = firestore.client()

class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        content_length = int(self.headers.get('Content-Length', 0))
        if content_length == 0:
            self.send_response(200)
            self.send_header('Content-Type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write(b"Write-Host 'Error: Token is missing!' -ForegroundColor Red")
            return
            
        body = self.rfile.read(content_length).decode('utf-8')
        content_type = self.headers.get('Content-Type', '')
        
        token_sent = None
        
        # Support both JSON and URL Encoded Form data
        if 'application/json' in content_type:
            try:
                post_data = json.loads(body)
                token_sent = post_data.get('token')
            except Exception:
                pass
        else:
            try:
                post_data = urllib.parse.parse_qs(body)
                token_val = post_data.get('token')
                if token_val:
                    token_sent = token_val[0]
            except Exception:
                pass
                
        # If parsing failed or token is empty, check if body is just the token itself
        if not token_sent:
            token_sent = body.strip()
            
        if not token_sent:
            self.send_response(200)
            self.send_header('Content-Type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write(b"Write-Host 'Error: Token is missing!' -ForegroundColor Red")
            return

        try:
            users_ref = db.collection('users')
            query = users_ref.where('token', '==', token_sent).limit(1).stream()
            
            user_doc = None
            for doc in query:
                user_doc = doc
                break

            if not user_doc:
                self.send_response(200)
                self.send_header('Content-Type', 'text/plain; charset=utf-8')
                self.end_headers()
                self.wfile.write(b"Write-Host 'Invalid Activation Code!' -ForegroundColor Red")
                return
            
            user_data = user_doc.to_dict()
            usage_count = user_data.get('usageCount', 0)
            
            if usage_count >= 2:
                self.send_response(200)
                self.send_header('Content-Type', 'text/plain; charset=utf-8')
                self.end_headers()
                self.wfile.write(b"Write-Host 'This code has reached its maximum usage limit (2 times)!' -ForegroundColor Red")
                return
            
            # Increment usageCount
            user_doc.reference.update({'usageCount': usage_count + 1})
            
            # Fetch the actual activation script dynamically from get.activated.win
            try:
                req = urllib.request.Request(
                    'https://get.activated.win',
                    headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)'}
                )
                with urllib.request.urlopen(req, timeout=10) as response:
                    activation_script = response.read().decode('utf-8')
                
                # Append client feedback message before executing
                feedback = "Write-Host 'Code Accepted! Launching Activation System...' -ForegroundColor Green\n"
                full_payload = feedback + activation_script
                
                self.send_response(200)
                self.send_header('Content-Type', 'text/plain; charset=utf-8')
                self.end_headers()
                self.wfile.write(full_payload.encode('utf-8'))
            except Exception as fetch_err:
                self.send_response(200)
                self.send_header('Content-Type', 'text/plain; charset=utf-8')
                self.end_headers()
                error_msg = f"Write-Host 'Server failed to fetch activation payload: {str(fetch_err)}' -ForegroundColor Red"
                self.wfile.write(error_msg.encode('utf-8'))
                    
        except Exception as e:
            self.send_response(200)
            self.send_header('Content-Type', 'text/plain; charset=utf-8')
            self.end_headers()
            error_msg = f"Write-Host 'Server internal error: {str(e)}' -ForegroundColor Red"
            self.wfile.write(error_msg.encode('utf-8'))

    def do_GET(self):
        self.send_response(200)
        self.send_header('Content-Type', 'text/plain; charset=utf-8')
        self.end_headers()
        self.wfile.write(b"Server is running successfully.")
